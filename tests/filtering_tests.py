...
originalSignalTime = (
    4 * np.sin(originalTime / 2)
    + 2.4 * np.cos(originalTime * 8)
    + 5 * np.cos(originalTime * 2)
)
...
class filteringModel(unittest.TestCase):
    ...
    def test__get_filter_params(self):
        ...
        filterType = "bandpass"
        _, _, _, _, w, h = get_filter_params(
            delta_time, lowFreq, highFreq, filterType, order
        )
        ...
        for i in range(len(w)):
            if h[i] > 1000 and w[i] > 0:  # broad spike
                self.assertFalse(
                    w[i] > 1.5 * lowFreq
                )  # not allowed to spike pas lowFreq
    ...
    def atest__filtering_time_freq(self):
        ...
        _, filteredSignalFreq, _, _ = filtering_time_freq(
            originalTime,
            originalSignalTime,
            delta_time,
            filterType,
