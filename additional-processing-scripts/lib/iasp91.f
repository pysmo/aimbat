...
c     call assign(lu,mode,ib(1:nb)//'.hed')
      call assign(lu,mode,dum(1:(nb+4)))
      return
      end
...
c
 ...
 1    go to (11,12,13),ns
 11   close(lu)
      open(lu,file=ia,status='old',form='formatted')
      rewind lu
      return
 12   inquire(file=ia,exist=exst)
      if(exst) go to 11
 ...
 2    go to (21,22,23),ns
 21   close(lu)
      open(lu,file=ia,status='old',form='unformatted')
      rewind lu
      return
 22   inquire(file=ia,exist=exst)
      if(exst) go to 21
...
c     call dasign(nin,-1,modnam(1:nb)//'.tbl',nasgr)
      dum=modnam(1:nb)//'.tbl'
      call dasign(nin,-1,dum(1:(nb+4)),nasgr)
c
      do 11 nph=1,2
