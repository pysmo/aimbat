...
AIMBAT never modifies input files. When you add a data source, AIMBAT reads the
...
!!! warning "Files must remain accessible"
    ...
    AIMBAT stores the path to each data file at import time. If you move, rename, or
    delete a file after importing it, AIMBAT will no longer be able to read waveform
    data for the associated seismogram. Keep data files in a stable location, or
    update the path in the database before moving them.
...
so the `--type` flag is required to distinguish them. See
[`aimbat data add`][aimbat._cli.data.cli_data_add] for the full set of flags,
including `--use-event`, `--use-station`, `--dry-run`, and `--no-snapshot`.

## Automatic snapshots

`data add` isn't SAC-specific: every event that receives at least one newly
imported seismogram is automatically snapshotted at the end of the command,
whatever `--type` produced it — giving you a baseline to roll back to before any
processing begins. Re-running `data add` on files already in the project does
not create a new snapshot, since nothing changed. Pass `--no-snapshot` to skip
...
The most common case — SAC files in one or more directories. Each file should
carry station and event headers, and a pick (typically stored in `T0`; see
[Aimbat Defaults](defaults.md) for the configurable pick header):

```bash
aimbat data add *.sac
```

Glob expansion (`*.sac`) is the primary way to select files. It's done by your
OS shell (bash, zsh, ...) before AIMBAT runs, so it works at a normal shell
prompt but not inside `aimbat shell`. Because AIMBAT deduplicates on import, you
can safely re-run the same command — files already in the project are skipped:
...
SAC files carry named time markers (`T0`–`T9`). AIMBAT reads one of these as the
initial phase pick (`t0` in AIMBAT's terminology). The default header is `T0`;
change it with `AIMBAT_SAC_PICK_HEADER`:
...
files — useful when SAC headers are incomplete or when you want to add metadata
first and link waveform files later.

### Supplying a missing event

If your SAC files do not carry event metadata (e.g. origin time or coordinates),
add it from a JSON file first, then link the SAC files to the resulting event:
...
The TUI file picker adds one file at a time — it cannot select multiple files or
a directory. For anything more than that, use the CLI or shell.
...
model: a seismogram is the record that ties together a waveform file, a station,
and an event. Removing a seismogram severs that link and drops the associated
data source entry. Events and stations are metadata containers shared across
seismograms — they are left in place even if no seismograms reference them any
more, since they may still be needed (or may have been added intentionally via
JSON).
...
To remove all seismograms associated with a station (across all events) and then
the station record:
...
In the TUI, select any row in the **Seismograms** tab and press `Enter` to open
