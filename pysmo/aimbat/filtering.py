...
def filtering_time_freq(
    ...
    lowFreq,
    order,
    runReversePass=False,
):
    _, originalSignalFreq = time_to_freq(originalTime, originalSignalTime, delta)

    # make filter, default is bandpass
    MULTIPLE = 0.7 * max(np.abs(originalSignalFreq))
    _, _, B, A, adjusted_w, adjusted_h = get_filter_params(
        delta, lowFreq, highFreq, filterType, order, MULTIPLE
    )
