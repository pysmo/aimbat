...
from numpy import array, concatenate, cos, linspace, mean, ones, pi, zeros
...
def zipFile(zipmode="gz"):
    ...
    if zipmode not in (None, "bz2", "gz"):
        raise ValueError(f"zipmode={zipmode:s} not in (bz2, gz)")
    if zipmode == "bz2":
        zfile = BZ2File
    elif zipmode == "gz":
        zfile = GzipFile
...
def _load_pickle(f):
    """Unpickle, falling back to latin1 for pickle files written under Python 2."""
    try:
...
def taper(data, taperwidth=0.1, tapertype="hanning"):
    ...
    if tapertype == "hanning":
        ...
        print(f"Unknown taper type: {tapertype:s} ! Exit")
        sys.exit(1)
    for i in range(npts):
        if i <= taperlen:
...
def windowIndex(saclist, reftimes, timewindow=(-5.0, 5.0), taperwindow=1.0):
    ...
    tw0, tw1 = timewindow
    twleft = tw0 - taperwindow * 0.5
    ntotal = int(round((tw1 - tw0 + taperwindow) / delta)) + 1
    nstart = [
        int(round((twleft + reftimes[i] - saclist[i].b) / delta))
...
def windowData(
    saclist, nstart, ntotal, taperwidth, tapertype="hanning", datatype="data"
):
    """
    ...
    for i in range(len(saclist)):
        ...
        if len(data) < ntotal:
            ...
            print(
                saclist[i].netsta
                + f": not enough sample around reftime. Pad left {len(zpada):d} right {len(zpadb):d} zeros"
            )
        data -= mean(data)
...
def loadData(ifiles, opts, para):
    ...
    print(
        f"Read {len(gsac.saclist):d} seismograms with sampling interval: {opts.delta:f}s"
    )
    return gsac
