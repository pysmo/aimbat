...
class PPConfig:
    ...
    def __init__(self):
        ...
        self.qweights = [
            float(val) for val in config.get("sachdrs", "qweights").split()
        ]
        # SAC plots
        self.figsize = [float(val) for val in config.get("sacplot", "figsize").split()]
        self.rectseis = [
            float(val) for val in config.get("sacplot", "rectseis").split()
        ]
...
class CCConfig:
    ...
    def __init__(self):
        ...
        print(
            f"Imported {xcorr_func.__name__:s} from {xcorr_modu.__name__:s} as cross-correlation method for ICCS"
        )
...
class MCConfig:
    ...
    def __init__(self):
        ...
        xcorr_modu, xcorr_func = _load_xcorr_func(func)
        print(
            f"Imported {xcorr_func.__name__:s} from {xcorr_modu.__name__:s} as cross-correlation method for MCCC"
        )
        self.xcorr = xcorr_func
...
def getParser():
    ...
    parser.add_option(
        ...
        f"Default is {reltime:d}, None, use absolute time.",
    )
    parser.add_option(
        "-u",
        "--upylim",
    ...
    parser.add_option(
        "-k", "--pick", action="store_true", dest="pick_on", help="Plot time picks."
    )
    parser.add_option(
        "-w", "--twin", action="store_true", dest="twin_on", help="Plot time window."
    )
    parser.add_option(
        "-x",
        "--xlimit",
