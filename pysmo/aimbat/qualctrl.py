...
def getOptions():
    ...
    parser.add_option(
        "-p", "--phase", dest="phase", type="str", help="Seismic phase name: P/S ."
    )
    parser.add_option(
        "-s",
        "--sortby",
        type="str",
        dest="sortby",
        help=f"Sort seismograms by i (file indices), or 0/1/2/3 (quality factor all/ccc/snr/coh), or t (time pick diff), or a given header (az/baz/dist..). Append - for decrease order, otherwise increase. Default is {sortby:s}.",
    )
    parser.add_option(
        "-t",
        "--twcorr",
        ...
        type="float",
        nargs=2,
        help=f"Time window for cross-correlation. Default is [{twcorr[0]:.1f}, {twcorr[1]:.1f}] s",
    )
    parser.add_option(
        "-g",
        "--savefig",
...
class PickPhaseMenuMore:
    ...
    def setLabels(self):
        ...
        if self.opts.filemode == "pkl":
            ...
            trans = mpl.transforms.blended_transform_factory(
                axstk.transAxes, axstk.transAxes
            )
            axstk.text(
                1,
                1.01,
    ...
    def plotStack(self):
        ...
        ppstk = ppk.PickPhase(
            self.gsac.stkdh, self.opts, self.axstk, stkybase, colorwave, 1
        )
        ...
        putil.pickLegend(
            self.axstk, pppara.npick, pppara.pickcolors, pppara.pickstyles, False
        )
        self.plotSpan()
    ...
    def summarize_sort(self):
        ...
        sortAxs["summary"].text(
            x1, y0 - dy * 1, "Sort in alphabetical order by filename"
        )

        sortAxs["summary"].text(x0, y0 - dy * 3, "QUALITY:")
        sortAxs["summary"].text(x0, y0 - dy * 4, "All: ")
        sortAxs["summary"].text(
            x1, y0 - dy * 4, "Weighted Ratio of all quality measures"
        )
        ...
        sortAxs["summary"].text(
            x1, y0 - dy * 11, "Beginning value of the independent variable"
        )
        sortAxs["summary"].text(x0, y0 - dy * 12, "E: ")
        sortAxs["summary"].text(
            x1, y0 - dy * 12, "Ending value of the independent variable"
        )
        sortAxs["summary"].text(x0, y0 - dy * 13, "Delta: ")
        sortAxs["summary"].text(
            x1, y0 - dy * 13, "Increment between evenly spaced samples"
        )
        sortAxs["summary"].text(x0, y0 - dy * 14, "STLA: ")
        sortAxs["summary"].text(
            x1, y0 - dy * 14, "Station latitude (deg, north positive)"
        )
        sortAxs["summary"].text(x0, y0 - dy * 15, "STLO: ")
        sortAxs["summary"].text(
            x1, y0 - dy * 15, "Station longitude (deg, east positive)"
        )
        ...
        sortAxs["summary"].text(
            x1, y0 - dy * 19, "Station to event great circle arc length (deg)"
        )
    ...
    def filter_connect(self):
        ...
        self.cidSelectFreq = (
            self.filterAxs["amVfreq"]
            .get_figure()
            .canvas.mpl_connect("button_press_event", self.getBandpassFreq)
        )

        # get order
        self.bnorder = mpl.widgets.RadioButtons(
            self.filterAxs["ordr"], (1, 2, 3, 4), active=1
        )
        self.cidorder = self.bnorder.on_clicked(self.getButterOrder)

        # get type of filter to use
        self.bnband = mpl.widgets.RadioButtons(
            self.filterAxs["band"], ("bandpass", "lowpass", "highpass")
        )
        self.cidband = self.bnband.on_clicked(self.getBandtype)

        # get reverse pass option
        self.bnreversepass = mpl.widgets.RadioButtons(
            self.filterAxs["reversepass"], ("yes", "no"), active=1
        )
    ...
    def getBandtype(self, event):
        ...
        if event == "bandpass":
            ...
            self.cidSelectFreq = (
                self.filterAxs["amVfreq"]
                .get_figure()
                .canvas.mpl_connect("button_press_event", self.getLowFreq)
            )
        elif event == "highpass":
            self.filterAxs["amVfreq"].figure.canvas.mpl_disconnect(self.cidSelectFreq)
            # set defaults
            ...
            self.cidSelectFreq = (
                self.filterAxs["amVfreq"]
                .get_figure()
                .canvas.mpl_connect("button_press_event", self.getHighFreq)
            )
    ...
    def getBandpassFreq(self, event):
        if event.inaxes == self.filterAxs["amVfreq"]:
            if self.opts.filterParameters[
                "advance"
            ]:  # low and high frequencies recorded
                self.opts.filterParameters["highFreq"] = event.xdata
                if (
                    ...
                    print(
                        "Value chose must be higher than lower frequency of %f"
                        % self.opts.filterParameters["lowFreq"]
                    )
            else:
                self.opts.filterParameters["lowFreq"] = event.xdata
    ...
    def modifyFilterTextLabels(self):
        ...
        self.filterAxs["Info"].text(
            0.1, 0.7, "Low Freq: " + str(self.opts.filterParameters["lowFreq"])
        )
        self.filterAxs["Info"].text(
            0.1, 0.4, "High Freq: " + str(self.opts.filterParameters["highFreq"])
        )
        self.filterAxs["Info"].text(
            0.1, 0.1, "Order: " + str(self.opts.filterParameters["order"])
        )
    ...
    def spreadButter(self):
        ...
        originalFreq, originalSignalFreq = ftr.time_to_freq(
            originalTime, originalSignalTime, self.opts.delta
        )
        filteredSignalTime, filteredSignalFreq, adjusted_w, adjusted_h = (
            ftr.filtering_time_freq(
                originalTime,
                originalSignalTime,
                self.opts.delta,
                ...
                self.opts.filterParameters["reversepass"],
            )
        )

        # PLOT TIME
        self.filterAxs["amVtime"].plot(
            originalTime, originalSignalTime, label="Original"
        )
        self.filterAxs["amVtime"].plot(
            originalTime, filteredSignalTime, label="Filtered"
        )
        ...
        # PLOT FREQUENCY
        self.filterAxs["amVfreq"].plot(
            originalFreq, np.abs(originalSignalFreq), label="Original"
        )
        self.filterAxs["amVfreq"].plot(
            originalFreq, np.abs(filteredSignalFreq), label="Filtered"
        )
        self.filterAxs["amVfreq"].plot(
            adjusted_w, adjusted_h, label="Butterworth Filter"
        )
    ...
    def getFilterAxes(self):
        ...
        filterAxs = {}
        self.figfilter.text(
            0.03, 0.95, "Butterworth Filter", {"weight": "bold", "size": 21}
        )
    ...
    def connect(self):
        ...
        self.cidpress = self.axstk.figure.canvas.mpl_connect(
    ...
    def sync(self, event):
        ...
        wh0, wh1 = self.opts.qcpara.twhdrs
        if self.ppstk.sacdh.gethdr(hdrfin) == -12345.0:
            print(
                "*** hfinal %s is not defined. Pick at array stack first! ***" % hdrfin
            )
            return
        self.syncPick()
        self.syncWind()
        ...
        for pp in self.ppm.pps:
            ...
            pp.resetWindow()
        print(
            "--> Sync final time picks and time window... You can now run CCFF to refine final picks."
        )
    ...
    def getWindow(self, hdr):
        ...
        if t0 == -12345.0:
            print(f"Header {hdr:s} not defined")
            return
    ...
    def ccim(self, event):
        ...
        shouldRun = tkinter.messagebox.askokcancel(
            "Will Erase Work!", "This will erase any picks past t1. \nAre you sure?"
        )
    ...
    def ccff(self, event):
        ...
        if self.gsac.stkdh.gethdr(hdrfin) == -12345.0:
        ...
        shouldRun = tkinter.messagebox.askokcancel(
            "Will Erase Work!",
            "This will erase any picks past t2 and will recalculate all t2 values. \nAre you sure?",
        )
...
def sortSeis(gsac, opts):
    ...
    if sortby == "i":  # by file indices
        ...
        if ipick == wpick:
            print(f"Same time pick: {ipick:s} and {wpick:s}. Exit")
            sys.exit()
        gsac.selist, gsac.delist = qualsort.sortSeisHeaderDiff(
            gsac.saclist, ipick, wpick, sortincrease
        )
